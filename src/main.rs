...
use pretzboard::weather::{self, Observation};
use pretzboard::flickr;
use pretzboard::{slideshow, PretzboardError, statusbar};
...
fn main() -> Result<(), PretzboardError> {
    env_logger::init();
    ...
    let mut window: GlfwWindow = WindowSettings::new("Pretzboard", [1366, 768])
        .exit_on_esc(true)
        //.fullscreen(true)
        .opengl(opengl)
        .build()
